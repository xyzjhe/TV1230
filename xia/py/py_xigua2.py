...
class Spider(Spider):
	...
	def categoryContent(self,tid,pg,filter,extend):
		...
		idTxt='电视剧'
		maximum=17
		url = 'https://www.xiguazx.com/api/cinema/filterv2/albums'
		if tid=='dianying':
			idTxt='电影'
		elif tid=='zongyi':
			idTxt='综艺'
			...
			url='https://www.xiguazx.com/api/userv2/follow/list?authorId={0}&sortType=desc&sortType=desc&cursor={1}'.format(self.userid,offset)
			maximum=12
		videos=[]
		if tid!='follow':
			offset=0 if int(pg)<2 else 18*int(pg)
			self.header['Referer']='https://www.xiguazx.com/cinema/filter/'.format(tid)
	...
	def detailContent(self,array):
		...
		act=aid[2]
		logo = aid[3]
		Url='https://www.xiguazx.com/api/albumv2/details?albumId={0}'.format(key)
		if len(aid)==5:
			Url='https://www.xiguazx.com/api/videov2/author/new_video_list?to_user_id={0}'.format(key)
		rsp = self.fetch(Url,headers=self.header)
		htmlTxt = rsp.text
		typeName=''
		area=''
	...
	def searchContent(self,key,quick):
		Url='https://www.xiguazx.com/api/searchv2/lvideo/{0}/0'.format(urllib.parse.quote(key))
		rsp = self.fetch(Url,headers=self.header)
		htmlTxt = rsp.text
		videos=self.get_list(html=htmlTxt)
		'''
		Url='https://www.xiguazx.com/api/searchv2/user/{0}/10'.format(urllib.parse.quote(key))
		rsp = self.fetch(Url,headers=self.header)
		htmlTxt1 = rsp.text
	...
	def playerContent(self,flag,id,vipFlags):
		result={}
		UrlId=id.split('_')
		Url='https://www.xiguazx.com/{0}'.format(UrlId[0])
		headers = {
	...
	def get_EpisodesList(self,jsonList):
		vodItems=[]
		for value in jsonList:
			vodItems.append(value['title']+"$"+'https://www.xiguazx.com/{0}?logTag=55abe18cfb733871bb04'.format(value['episodeId']))
		return vodItems
	config = {
		"player": {},
		"filter": {}
	...
	header = {
		...
		"Referer": 'https://www.xiguazx.com/cinema/filter/dianshiju/',
